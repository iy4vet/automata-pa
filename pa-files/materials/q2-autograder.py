...
def autograde(input_dataset_filepath):
    base, ext = os.path.splitext(input_dataset_filepath)
    output_filepath = f"{base}_output.txt"
    sol_filepath = f"{base}_sol.txt"

    if not os.path.exists(input_dataset_filepath):
        print(f"Error: Input file '{input_dataset_filepath}' not found.")
        return 0

    try:
        subprocess.run(
            ["python", "./Question-2/q2.py", input_dataset_filepath],
            check=True
        )
    except subprocess.CalledProcessError as e:
        print(f"Error: q2.py failed with exit code {e.returncode}")
        return 0
...
print("Enter Input Dataset File Path:")
input_dataset_filepath = input()
score = autograde(input_dataset_filepath)
print(f"Input: {input_dataset_filepath}")
